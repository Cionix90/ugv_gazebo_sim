...
def create_yaml(context: LaunchContext, robot_name,joystic_teleop):
    ...
    if(robot_name_str != ""):  
        path = os.path.join(get_package_share_path("scout_description"),"config", "sim_controllers.yaml")
        ...
        file_name = path = os.path.join(get_package_share_path("scout_description"),"config", f"{robot_name_str}_controllers.yaml")
    ...
    # new_file = {robot_name: file[] }
        ...
        teleop_node = Node(
            ...
            parameters=[os.path.join(get_package_share_path("scout_description"),"config", "teleop_conf.yaml")],
            remappings=[
...
def generate_launch_description():
    ...
    hunter_robot_path = get_package_share_path("scout_description")
    hunter_robot_path = os.path.join(hunter_robot_path,"urdf", "scout_v2.xacro")
    ...
    robot_description = ParameterValue(
        Command(["xacro ",LaunchConfiguration("scout_urdf"), " tf_prefix:=",robot_name]),
        value_type=str
    )
